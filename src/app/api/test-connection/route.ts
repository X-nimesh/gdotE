...
export async function POST(req: NextRequest) {
  const { connectionString, cosmosKey, cosmosDatabase, cosmosCollection } = await req.json();
  
  if (!connectionString) {
    return NextResponse.json({ success: false, error: "Missing connection string" }, { status: 400 });
  }
  ...
  try {
    ...
    const queryPromise = client.submit(testQuery).then((result: any) => result.toArray());
